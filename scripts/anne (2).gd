...
# npc name
@export var npc_name = "Sister Cindy"
...
func dialog():
	...
	# Show dialog popup
	dialog_popup.message = "HO NO MO!!! IT'S SLUT SHAMING TIME!"
	dialog_popup.response = "[C] What?  [B] So true, bestie."
...
func fakeprocess():
	while true:
		if dialog_state == 1 and Input.is_action_pressed('KEY_C'):
			...
			dialog_state = 2
			# Show dialog popup
			dialog_popup.message = "Listen to Jesus, and you'll understand."
			dialog_popup.response = "[A] I'm calling Campo."
			fakeprocess()
			break
			...
			dialog_state = 3
			# Show dialog popup
			dialog_popup.message = "Thank you!"
			dialog_popup.response = "[A] You're welcome!"
			fakeprocess()
			break
